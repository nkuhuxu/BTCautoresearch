...
def model_fn(train_days, train_log_prices, test_days):
    ...
    if n_trend > 1:
        x = np.arange(n_trend)
        w = np.exp(1.25 * x / n_trend)  # More weight on recent points
        slope, _ = np.polyfit(x, residuals[-n_trend:], 1, w=w)
        trend = slope * dt / 2.98  # Scale by days
    else:
        trend = 0.0

    # Blend: deviation decays, trend continues with light damping
    trend_damping = np.exp(-0.525 * dt / half_life_trend)  # Very light damping
    return formula(test_days, a, b, c, d) + 1.062 * deviation * decay + 1.805 * trend * trend_damping
