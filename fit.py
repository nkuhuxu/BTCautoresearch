...
def model_fn(train_days, train_log_prices, test_days):
    ...
    if n_trend > 1:
        x = np.arange(n_trend)
        w = np.exp(0.5 * x / n_trend)  # More weight on recent points
        slope, _ = np.polyfit(x, residuals[-n_trend:], 1, w=w)
        trend = slope * dt / 10.0  # Scale by days
    else:
        trend = 0.0

    # Blend: deviation decays, trend continues with small damping
    # No damping on trend (continues indefinitely)
    return formula(test_days, a, b, c, d) + deviation * decay + trend
