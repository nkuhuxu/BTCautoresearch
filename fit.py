...
def model_fn(train_days, train_log_prices, test_days):
    ...
    if n_trend > 1:
        x = np.arange(n_trend)
        w = np.exp(0.95 * x / n_trend)  # More weight on recent points
        slope, _ = np.polyfit(x, residuals[-n_trend:], 1, w=w)
        trend = slope * dt / 4.0  # Scale by days
    else:
        trend = 0.0

    # Blend: deviation decays, trend continues with light damping
    trend_damping = np.exp(-0.46 * dt / half_life)  # Very light damping
    return formula(test_days, a, b, c, d) + 1.05 * deviation * decay + 1.12 * trend * trend_damping
